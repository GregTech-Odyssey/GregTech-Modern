...
public class MachineRecipeLoader {
    ...
    private static void registerPrimitiveBlastFurnaceRecipes() {
        ...
        PRIMITIVE_BLAST_FURNACE_RECIPES.recipeBuilder("steel_from_charcoal_block_wrought")
                ...
                .outputItems(dust, DarkAsh, 2).duration(7200).save();
        PRIMITIVE_BLAST_FURNACE_RECIPES.recipeBuilder("steel_from_coke_block_wrought").inputItems(block, WroughtIron)
                .inputItems(block, Coke).outputItems(block, Steel).outputItems(dust, Ash).duration(5400).save();
    }
    ...
    private static void registerStoneBricksRecipes() {
        ...
        for (int i = 0; i < stones.size(); i++) {
            ResourceLocation bricksId = GTUtil.ITEM_ID.apply(bricks.get(i).getItem());
    ...
    private static void registerAssemblerRecipes() {
        ...
        ASSEMBLER_RECIPES.recipeBuilder("casing_bronze_bricks").EUt(16).inputItems(plate, Bronze, 6)
                ...
                .duration(50).addMaterialInfo(true).save();
        ...
        ASSEMBLER_RECIPES.recipeBuilder("hull_mv_annealed").duration(50).EUt(16)
                ...
                .inputFluids(Polyethylene, L * 2).outputItems(GTMachines.HULL[MV]).save();
    ...
    private static void registerSmoothRecipe(List<ItemStack> roughStack,
        ...
        for (int i = 0; i < roughStack.size(); i++) {
            ResourceLocation stoneId = GTUtil.ITEM_ID.apply(stoneStack.get(i).getItem());
    ...
    private static void registerCobbleRecipe(List<ItemStack> stoneStack,
        ...
        for (int i = 0; i < stoneStack.size(); i++) {
            ResourceLocation cobbleId = GTUtil.ITEM_ID.apply(cobbleStack.get(i).getItem());
    ...
    private static void registerBricksRecipe(List<ItemStack> polishedStack,
        ...
        for (int i = 0; i < polishedStack.size(); i++) {
            ResourceLocation brickId = GTUtil.ITEM_ID.apply(brickStack.get(i).getItem());
    ...
    private static void registerMossRecipe(List<ItemStack> regularStack,
        ...
        for (int i = 0; i < regularStack.size(); i++) {
            ResourceLocation mossId = GTUtil.ITEM_ID.apply(mossStack.get(i).getItem());
    ...
    private static void registerNBTRemoval() {
        for (MachineDefinition chest : GTMachines.SUPER_CHEST) {
            if (chest != null) {
                VanillaRecipeHelper.addShapelessNBTClearingRecipe("super_chest_nbt_" + chest.getTier(),
        ...
        for (MachineDefinition tank : GTMachines.SUPER_TANK) {
            if (tank != null) {
                VanillaRecipeHelper.addShapelessNBTClearingRecipe("super_tank_nbt_" + tank.getTier(),
        ...
        for (MachineDefinition chest : GTMachines.QUANTUM_CHEST) {
            if (chest != null) {
                VanillaRecipeHelper.addShapelessNBTClearingRecipe("quantum_chest_nbt_" + chest.getTier(),
                        chest.asStack(), chest.asStack());
            }
        }
        ...
        for (MachineDefinition tank : GTMachines.QUANTUM_TANK) {
            if (tank != null) {
                VanillaRecipeHelper.addShapelessNBTClearingRecipe("quantum_tank_nbt_" + tank.getTier(),
                        tank.asStack(), tank.asStack());
            }
        }
    ...
    private static void registerHatchConversion() {
        ...
        for (int tier : GTMachineUtils.MULTI_HATCH_TIERS) {
            ...
            VanillaRecipeHelper.addShapedRecipe(
                    "fluid_hatch_4x_output_to_input_" + tierName,
                    importHatch4x.asStack(), "d", "B",
                    'B', exportHatch4x.asStack());
            VanillaRecipeHelper.addShapedRecipe(
                    "fluid_hatch_4x_input_to_output_" + tierName,
                    exportHatch4x.asStack(), "d", "B",
                    'B', importHatch4x.asStack());

            VanillaRecipeHelper.addShapedRecipe(
                    "fluid_hatch_9x_output_to_input_" + tierName,
                    importHatch9x.asStack(), "d", "B",
                    'B', exportHatch9x.asStack());
            VanillaRecipeHelper.addShapedRecipe(
                    "fluid_hatch_9x_input_to_output_" + tierName,
