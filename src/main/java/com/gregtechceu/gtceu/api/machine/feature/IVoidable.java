...
public interface IVoidable extends IMachineFeature {
    ...
    static void attachConfigurators(ConfiguratorPanel configuratorPanel, IVoidable controller) {
        configuratorPanel.attachConfigurators(new FancySelectorConfigurator<>(VoidingMode.VALUES, controller.getVoidingMode(), controller::setVoidingMode).setLatched(m -> m != VoidingMode.VOID_NONE).syncFrom(controller::getVoidingMode).setTooltip(m -> (List) LangHandler.getMultiLang(m.localeName)));
    }
