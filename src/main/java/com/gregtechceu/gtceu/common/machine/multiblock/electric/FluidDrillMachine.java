...
public class FluidDrillMachine extends WorkableElectricMultiblockMachine {

    private final int t;

    public FluidDrillMachine(MetaMachineBlockEntity holder, int tier) {
        super(holder);
        this.t = tier;
    }
    ...
    public void addDisplayText(List<Component> textList) {
        if (isFormed()) {
            ...
            if (getRecipeLogic().getVeinFluid() != null) {
                ...
                // Fluid amount
                Component amountInfo = Component.literal(FormattingUtil.formatNumbers(getRecipeLogic().getFluidToProduce() * 20L / FluidDrillLogic.MAX_PROGRESS) + " mB/s").withStyle(ChatFormatting.BLUE);
