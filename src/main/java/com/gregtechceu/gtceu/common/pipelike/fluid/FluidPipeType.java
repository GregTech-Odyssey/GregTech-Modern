...
public enum FluidPipeType implements IMaterialPipeType<FluidPipeProperties> {
    ...
    public FluidPipeProperties modifyProperties(FluidPipeProperties fluidPipeData) {
        return new FluidPipeProperties(fluidPipeData.getThroughput() * capacityMultiplier * channels);
    }
    ...
    public PipeModel createPipeModel(Material material) {
        ...
        if (channels == 9) {
            return new PipeModel(thickness, () -> GTCEu.id("block/pipe/pipe_non_side"), () -> GTCEu.id("block/pipe/pipe_%s_in".formatted(name)), null, null);
        } else if (channels == 4) {
