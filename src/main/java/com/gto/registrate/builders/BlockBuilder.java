...
import com.gto.registrate.providers.loot.RegistrateBlockLootTables;
import com.gto.registrate.providers.loot.RegistrateLootTableProvider;
...
public class BlockBuilder<T extends Block, P> extends AbstractBuilder<Block, T, P, BlockBuilder<T, P>> {
     ...
     */
    public static <T extends Block, P> BlockBuilder<T, P> create(AbstractRegistrate<?> owner, P parent, String name, NonNullFunction<BlockBehaviour.Properties, T> factory) {
        return new BlockBuilder<>(owner, parent, name, factory, BlockBehaviour.Properties::of)
     ...
     * configuration.
     * <p>
     * The item will have no lang entry (since it would duplicate the block's) and a simple block item model (via
     ...
     * {@link RegistrateBlockstateProvider#simpleBlock(Block)}). This is the default, so it is generally not necessary
