...
public interface Builder<R, T extends R, P, S extends Builder<R, T, P, S>> {
     ...
     * callbacks.
     * <p>
     * Builders which have had this method used on them (or another method which calls this one, such as
     * {@link EntityBuilder#spawnEgg(int, int)}), <strong>must</strong> be registered, via
     ...
     */
    S onRegister(NonNullConsumer<? super T> callback);
    ...
    /**
     * Register the entry and return the parent object. The {@link RegistryObject} will be created but not returned. It
     * can be retrieved later with {@link AbstractRegistrate#get(ResourceKey)} or
