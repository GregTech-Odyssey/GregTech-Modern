...
import com.gto.registrate.providers.loot.RegistrateEntityLootTables;
import com.gto.registrate.providers.loot.RegistrateLootTableProvider.LootType;
...
public class EntityBuilder<T extends Entity, P> extends AbstractBuilder<EntityType<?>, EntityType<T>, P, EntityBuilder<T, P>> {
     ...
     */
    public static <T extends Entity, P> EntityBuilder<T, P> create(AbstractRegistrate<?> owner, P parent, String name, EntityType.EntityFactory<T> factory,
                                                                   MobCategory classification) {
        return new EntityBuilder<>(owner, parent, name, factory, classification)
                .defaultLang();
    }
    ...
    protected EntityBuilder(AbstractRegistrate<?> owner, P parent, String name, EntityType.EntityFactory<T> factory, MobCategory classification) {
        super(owner, parent, name, ForgeRegistries.Keys.ENTITY_TYPES);
        ...
        OneTimeEventReceiver.addModListener(getOwner(), EntityRenderersEvent.RegisterRenderers.class, evt -> {
            ...
            if (renderer != null) {
                try {
                    ...
                    throw new IllegalStateException("Failed to register renderer for Entity " + getName(), e);
                }
            }
        });
    }
    ...
    public ItemBuilder<? extends SpawnEggItem, EntityBuilder<T, P>> spawnEgg(int primaryColor, int secondaryColor) {
        var sup = asSupplier();
        return getOwner().item(this, getName() + "_spawn_egg", p -> new ForgeSpawnEggItem((Supplier<EntityType<? extends Mob>>) (Supplier) sup, primaryColor, secondaryColor, p)).tab(CreativeModeTabs.SPAWN_EGGS)
    ...
    @Override
    protected RegistryEntry<EntityType<T>> createEntryWrapper(ResourceKey<EntityType<T>> key) {
