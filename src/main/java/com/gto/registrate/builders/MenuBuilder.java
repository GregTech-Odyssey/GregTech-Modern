...
import com.gto.registrate.util.entry.MenuEntry;
...
public class MenuBuilder<T extends AbstractContainerMenu, S extends Screen & MenuAccess<T>, P> extends AbstractBuilder<MenuType<?>, MenuType<T>, P, MenuBuilder<T, S, P>> {
    ...
    public MenuBuilder(AbstractRegistrate<?> owner, P parent, String name, MenuFactory<T> factory, NonNullSupplier<ScreenFactory<T, S>> screenFactory) {
        this(owner, parent, name, (type, windowId, inv, $) -> factory.create(type, windowId, inv), screenFactory);
    }

    public MenuBuilder(AbstractRegistrate<?> owner, P parent, String name, ForgeMenuFactory<T> factory, NonNullSupplier<ScreenFactory<T, S>> screenFactory) {
        super(owner, parent, name, ForgeRegistries.Keys.MENU_TYPES);
    ...
    @Override
    protected RegistryEntry<MenuType<T>> createEntryWrapper(ResourceKey<MenuType<T>> key) {
