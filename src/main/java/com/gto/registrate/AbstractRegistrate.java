...
import com.gto.registrate.builders.Builder;
import com.gto.registrate.builders.EnchantmentBuilder;
import com.gto.registrate.builders.EnchantmentBuilder.EnchantmentFactory;
import com.gto.registrate.builders.EntityBuilder;
import com.gto.registrate.builders.FluidBuilder;
import com.gto.registrate.builders.ItemBuilder;
import com.gto.registrate.builders.MenuBuilder;
import com.gto.registrate.builders.MenuBuilder.ForgeMenuFactory;
import com.gto.registrate.builders.MenuBuilder.MenuFactory;
import com.gto.registrate.builders.MenuBuilder.ScreenFactory;
import com.gto.registrate.builders.NoConfigBuilder;
import com.gto.registrate.providers.ProviderType;
import com.gto.registrate.providers.RegistrateDataProvider;
import com.gto.registrate.providers.RegistrateProvider;
import com.gto.registrate.util.CreativeModeTabModifier;
import com.gto.registrate.util.DebugMarkers;
...
public abstract class AbstractRegistrate<S extends AbstractRegistrate<S>> {
    ...
    private static class Registration<R, T extends R> {
        ...
        private Registration(ResourceLocation name, ResourceKey<? extends Registry<R>> type, List<NonNullConsumer<? super T>> callbacks, NonNullSupplier<? extends T> creator, NonNullFunction<ResourceKey<T>, ? extends RegistryEntry<T>> entryFactory) {
            this.key = ResourceKey.create(type, name);
            this.callbacks = callbacks;
            this.creator = creator;
            this.delegate = entryFactory.apply((ResourceKey) key);
        }
        ...
        private void register(RegisterEvent event) {
            ...
            if (forgeRegistry != null) {
                forgeRegistry.register(key.location(), entry);
            } else {
                Registry.register(event.getVanillaRegistry(), key, entry);
            }
            delegate.bound(entry);
    ...
    @Nullable
    public ResourceKey<CreativeModeTab> defaultCreativeModeTab = CreativeModeTabs.SEARCH;
     ...
     * alternative language loaders, such as forgelin. Defaults to the event bus in
    ...
    public <R, T extends R> RegistryEntry<T> get(String name, ResourceKey<? extends Registry<R>> type) {
        return this.<R, T>getRegistration(name, type).delegate;
    }
     ...
     * @return A {@link RegistryEntry} which will supply the requested entry, if it exists, otherwise the
     *         {@link RegistryEntry#empty() empty entry}
     */
    public <R, T extends R> RegistryEntry<T> getOptional(String name, ResourceKey<? extends Registry<R>> type) {
        ...
        return reg == null ? RegistryEntry.empty() : reg.delegate;
    }
    ...
    public <R> Collection<RegistryEntry<R>> getAll(ResourceKey<? extends Registry<R>> type) {
        return registrations.get(type).values().stream().map(r -> (RegistryEntry<R>) r.delegate).toList();
    }
    ...
    public <P extends RegistrateProvider, R> S setDataGenerator(String entry, ResourceKey<? extends Registry<R>> registryType, ProviderType<? extends P> type, NonNullConsumer<? extends P> cons) {
        if (!GTCEu.isDataGen()) return self();
        @SuppressWarnings("null")
        Consumer<? extends RegistrateProvider> existing = datagensByEntry.put(type, Pair.of(entry, registryType), cons);
        if (existing != null) {
    ...
    @SuppressWarnings("null")
    private Optional<Pair<String, ResourceKey<? extends Registry<?>>>> getEntryForGenerator(ProviderType<?> type, Consumer<? extends RegistrateProvider> generator) {
        for (Map.Entry<Pair<String, ResourceKey<? extends Registry<?>>>, Consumer<? extends RegistrateProvider>> e : datagensByEntry.get(type).entrySet()) {
    ...
    public S defaultCreativeTab(RegistryEntry<CreativeModeTab> creativeModeTab) {
        defaultCreativeModeTab = creativeModeTab.getKey();
        return self();
    }

    public S removeDefaultCreativeTab() {
        defaultCreativeModeTab = null;
        return self();
    }
    ...
    public <R, T extends R> RegistryEntry<T> register(String name, ResourceKey<? extends Registry<R>> type, NonNullSupplier<? extends T> creator) {
        return register(name, type, Collections.emptyList(), creator, RegistryEntry::new);
    }

    public <R, T extends R> RegistryEntry<T> register(String name, ResourceKey<? extends Registry<R>> type, NonNullSupplier<? extends T> creator, NonNullFunction<ResourceKey<T>, ? extends RegistryEntry<T>> entryFactory) {
        return register(name, type, Collections.emptyList(), creator, entryFactory);
    }

    public <R, T extends R> RegistryEntry<T> register(String name, ResourceKey<? extends Registry<R>> type, List<NonNullConsumer<? super T>> callbacks, NonNullSupplier<? extends T> creator, NonNullFunction<ResourceKey<T>, ? extends RegistryEntry<T>> entryFactory) {
        Registration<R, T> reg = new Registration<>(new ResourceLocation(modid, name), type, callbacks, creator, entryFactory);
    ...
    public <T extends Item, P> ItemBuilder<T, P> item(P parent, String name, NonNullFunction<Item.Properties, T> factory) {
        return ItemBuilder.create(this, parent, name, factory);
    }
    ...
    public <T extends Entity, P> EntityBuilder<T, P> entity(P parent, String name, EntityFactory<T> factory, MobCategory classification) {
        return EntityBuilder.create(this, parent, name, factory, classification);
    }
    ...
    public <P> FluidBuilder<ForgeFlowingFluid.Flowing, P> fluid(P parent, String name, ResourceLocation stillTexture, ResourceLocation flowingTexture) {
        return FluidBuilder.create(this, parent, name, stillTexture, flowingTexture);
    }
    ...
    public <P> FluidBuilder<ForgeFlowingFluid.Flowing, P> fluid(P parent, String name, ResourceLocation stillTexture, ResourceLocation flowingTexture, FluidBuilder.FluidTypeFactory typeFactory) {
        return FluidBuilder.create(this, parent, name, stillTexture, flowingTexture, typeFactory);
    }
    ...
    public <P> FluidBuilder<ForgeFlowingFluid.Flowing, P> fluid(P parent, String name, ResourceLocation stillTexture, ResourceLocation flowingTexture, NonNullSupplier<FluidType> fluidType) {
        return FluidBuilder.create(this, parent, name, stillTexture, flowingTexture, fluidType);
    }
    ...
    public <T extends ForgeFlowingFluid, P> FluidBuilder<T, P> fluid(P parent, String name, ResourceLocation stillTexture, ResourceLocation flowingTexture,
                                                                     NonNullFunction<ForgeFlowingFluid.Properties, T> fluidFactory) {
        return FluidBuilder.create(this, parent, name, stillTexture, flowingTexture, fluidFactory);
    }

    public <T extends ForgeFlowingFluid, P> FluidBuilder<T, P> fluid(P parent, String name, ResourceLocation stillTexture, ResourceLocation flowingTexture,
                                                                     FluidBuilder.FluidTypeFactory typeFactory, NonNullFunction<ForgeFlowingFluid.Properties, T> fluidFactory) {
        return FluidBuilder.create(this, parent, name, stillTexture, flowingTexture, typeFactory, fluidFactory);
    }

    public <T extends ForgeFlowingFluid, P> FluidBuilder<T, P> fluid(P parent, String name, ResourceLocation stillTexture, ResourceLocation flowingTexture,
                                                                     NonNullSupplier<FluidType> fluidType, NonNullFunction<ForgeFlowingFluid.Properties, T> fluidFactory) {
        return FluidBuilder.create(this, parent, name, stillTexture, flowingTexture, fluidType, fluidFactory);
    }
    ...
    public <T extends AbstractContainerMenu, SC extends Screen & MenuAccess<T>, P> MenuBuilder<T, SC, P> menu(P parent, String name, MenuFactory<T> factory, NonNullSupplier<ScreenFactory<T, SC>> screenFactory) {
        return new MenuBuilder<>(this, parent, name, factory, screenFactory);
    }
    ...
    public <T extends Enchantment, P> EnchantmentBuilder<T, P> enchantment(P parent, String name, EnchantmentCategory type, EnchantmentFactory<T> factory) {
        return EnchantmentBuilder.create(this, parent, name, type, factory);
    }
